...
export const startUpdateProductDetails = (_id, formData, setErrors, history) => {
    return (
        (dispatch) => {
            axios.put(`http://localhost:3050/api/products/${_id}`, formData, {
                headers: {
                    'x-auth': localStorage.getItem('token')
                }
            })
...
export const startDeleteProduct = (_id) => {
    return (
        (dispatch) => {
            axios.delete(`http://localhost:3050/api/products/${_id}`, {
                headers: {
                    'x-auth': localStorage.getItem('token')
                }
            })
