...
export const startGetTasks = () => {
    return (
        (dispatch) => {
            axios.get('http://localhost:3050/api/tasks', {
                headers: {
...
export const startPostTask = (formData, resetForm, setErrors) => {
    return (
        (dispatch) => {
            axios.post('http://localhost:3050/api/tasks', formData, {
                headers: {
                    'x-auth': localStorage.getItem('token')
                }
            })
...
export const startUpdateTask = (_id, formData, resetForm, setErrors, toggleEdit) => {
    return (
        (dispatch) => {
            axios.put(`http://localhost:3050/api/tasks/${_id}`, formData, {
                headers: {
                    'x-auth': localStorage.getItem('token')
                }
            })
...
export const startDeleteTask = (_id) => {
    return (
        (dispatch) => {
            axios.delete(`http://localhost:3050/api/tasks/${_id}`, {
                headers: {
                    'x-auth': localStorage.getItem('token')
                }
            })
